...
$(".slider .slide").each(function(){
	...
	////web.archive.org/web/20131213144140/http://slide 1
	
	$("#s51_text").css({"display" : "none", "opacity": 0});
	$("#s52_text").css({"display" : "none", "opacity": 0});
					
	$("#s1").css("display" , "block");
	$("#s1_text").css({"left" : "350px","display" : "block"})
	$("#s1").animate(
		{
			opacity: 1
		},1100)
	$("#s1_text").animate(
	{
		...
		//web.archive.org/web/20131213144140/http://slide 2 - trransit
		$("#s1_text").animate(
		{
			opacity: 0,
			left : "20px"
		...
		},1300,function ()
		{
			...
			$("#s2_text").css({"left" : "10px","display" : "block"})
			...
			//web.archive.org/web/20131213144140/http://slide 3
			
			$("#s2_text").css("display" , "none");
			...
			},1800,function()
			{
				...
				$("#s3_text").css({"left" : "10px","display" : "block"})
				...
				//web.archive.org/web/20131213144140/http://slide 4
				
				$("#s3_text").css("display" , "none");
				...
				$("#s4").animate(
				{
					...
					$("#s4_text").css({"left" : "10px","display" : "block"})
					...
					$("#s51_text").css("display" , "block");
					$("#s52_text").css("display" , "block");
...
;(function($){$.fn.diapo = function(opts, callback) {
	/*
	var defaults = {
		selector			: 'div',	//web.archive.org/web/20131213144140/http://target element
		
		fx					: 'simpleFade',
//'random','simpleFade','curtainTopLeft','curtainTopRight','curtainBottomLeft','curtainBottomRight','curtainSliceLeft','curtainSliceRight','blindCurtainTopLeft','blindCurtainTopRight','blindCurtainBottomLeft','blindCurtainBottomRight','blindCurtainSliceBottom','blindCurtainSliceTop','stampede','mosaic','mosaicReverse','mosaicRandom','mosaicSpiral','mosaicSpiralReverse','topLeftBottomRight','bottomRightTopLeft','bottomLeftTopRight','bottomLeftTopRight','scrollLeft','scrollRight','scrollTop','scrollBottom','scrollHorz'
		
//web.archive.org/web/20131213144140/http://you can also use more than one effect: 'curtainTopLeft, mosaic, bottomLeftTopRight'

		mobileFx			: '',	//web.archive.org/web/20131213144140/http://leave empty if you want to display the same effect on mobile devices and on desktop etc.

		slideOn				: 'random',	//next, prev, random: decide if the transition effect will be applied to the current (prev) or the next slide
				
		gridDifference		: 250,	//web.archive.org/web/20131213144140/http://to make the grid blocks slower than the slices, this value must be smaller than transPeriod
		
		easing				: 'easeInOutExpo',	//web.archive.org/web/20131213144140/http://for the complete list http://web.archive.org/web/20131213144140/http://jqueryui.com/demos/effect/easing.html
		
		mobileEasing		: '',	//web.archive.org/web/20131213144140/http://leave empty if you want to display the same easing on mobile devices and on desktop etc.
		...
		piePosition			: 'top:5px; right:5px',	//web.archive.org/web/20131213144140/http://this option accepts any CSS value
		...
		slicedRows			: 8,	//web.archive.org/web/20131213144140/http://if 0 the same value of rows
		
		slicedCols			: 12,	//web.archive.org/web/20131213144140/http://if 0 the same value of cols
		
		time				: 3000,	//web.archive.org/web/20131213144140/http://milliseconds between the end of the sliding effect and the start of the nex one
		
		transPeriod			: 1500,	//web.archive.org/web/20131213144140/http://lenght of the sliding effect in milliseconds
		
		autoAdvance			: true,	//true, false
		
		mobileAutoAdvance	: true, //web.archive.org/web/20131213144140/http://truem false. Auto-advancing for mobile devices
	...
	function isMobile() {	//web.archive.org/web/20131213144140/http://sniff a mobile browser
...
//web.archive.org/web/20131213144140/http://Define some difference if is a mobile device or not
	var clickEv,
		autoAdv,
		navigation,
		navHover,
...
//web.archive.org/web/20131213144140/http://The slideshow starts when all the images are loaded
	...
	if(elem.length!=0){
		...
		selector.wrapInner('<div class="pix_relativize" style="width:'+w+'px; height:'+h+'px" />');	//web.archive.org/web/20131213144140/http://wrap a div for the position of absolute elements
		var amountSlide = selector.length;    //web.archive.org/web/20131213144140/http://how many sliders
		
		var nav;	//nextSlide(nav)
		
		function imgFake() {	//web.archive.org/web/20131213144140/http://this function replace elements such as iframes or objects with an image stored in data-fake attribute
			$('*[data-fake]',elem).each(function(){
				var t = $(this);
				var imgFakeUrl = t.attr('data-fake');
				var imgFake = new Image();
				imgFake.src = imgFakeUrl;
				t.after($(imgFake).attr('class','imgFake'));	//web.archive.org/web/20131213144140/http://the image has class imgFake
				var clone = t.clone();
		...
		if(opts.hover==true){	//web.archive.org/web/20131213144140/http://if the option "hover" is true I stop the slideshow on mouse over and I resume it on mouse out
			if(!isMobile()){
				elem.hoverIntent({	
					over: function(){
							elem.addClass('stopped');
		...
		if(navHover==true){	//web.archive.org/web/20131213144140/http://if the option is true I show the next and prev button only on mouse over
			if(isMobile()){
		...
		$('#pix_stop').live('click',function(){	//web.archive.org/web/20131213144140/http://stop function
			elem.diapoStop();
		});
		...
		$('#pix_play').live('click',function(){	//web.archive.org/web/20131213144140/http://play function
			elem.diapoPlay();
		});
	
		if(opts.pauseOnClick==true){	//web.archive.org/web/20131213144140/http://if option is true I stop the slideshow if the user clicks on the slider
		...
		$('img', elem).each( function() { 
			allImg.push($(this).attr('src'));	//web.archive.org/web/20131213144140/http://all the images are pushed in the array
		});
		if (!$.browser.msie) {	//web.archive.org/web/20131213144140/http://sorry IE8- has some problem with this
		...
		loadimages(allImg,nextSlide);	//web.archive.org/web/20131213144140/http://when all the images in the array are loaded nextSlide function starts
		
	}
	
	
		function shuffle(arr) {	//web.archive.org/web/20131213144140/http://to randomize the effect
			for(
		...
		function isInteger(s) {	//web.archive.org/web/20131213144140/http://to check if a number is integer
			return Math.ceil(s) == Math.floor(s);
		}	
	
		if (($.browser.msie && $.browser.version < 9) || opts.loader == 'bar') {	//web.archive.org/web/20131213144140/http://IE8- has some problems with canvas, I prefer to use a simple loading bar in CSS
		...
		if(opts.loader=='none' || autoAdv==false) {	//web.archive.org/web/20131213144140/http://hide the loader if you want
	...
	function nextSlide(nav){    //web.archive.org/web/20131213144140/http://funzione per il fading delle immagini
		elem.addClass('diaposliding');	//web.archive.org/web/20131213144140/http://aggiunge una classe che mi dice che il fading è in corso
		
		var vis = parseFloat($('> '+opts.selector +'.diapocurrent',elem).not('#pix_canvas').not('#pix_canvas_wrap').not('#pix_next').not('#pix_prev').not('#pix_commands').index());    //web.archive.org/web/20131213144140/http://la variabile è il numero del div partendo da 0

		if(nav>0){    //web.archive.org/web/20131213144140/http://se siamo all'ultimo div o se ancora non ho creato nessun div
			var i = nav-1;
		} else if (vis == amountSlide-1) { 
			var i = 0;
		} else {    //web.archive.org/web/20131213144140/http://altrimenti l'indice è l'id corrent più uno, quindi il div successivo
			var i = vis+1;
		}
		...
		switch(fx){
			...
			var cycle = 0;
			var blocks = rows*cols;	//web.archive.org/web/20131213144140/http://number of squares
			var leftScrap = w-(Math.floor(w/cols)*cols);	//web.archive.org/web/20131213144140/http://difference between rounded widths and total width
			var topScrap = h-(Math.floor(h/rows)*rows);	//web.archive.org/web/20131213144140/http://difference between rounded heights and total height
...
/*
     FILE ARCHIVED ON 14:41:40 Dec 13, 2013 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 17:53:35 May 02, 2017.
